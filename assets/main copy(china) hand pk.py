...
# ============================================================
#  MediaPipe Hand Landmarker（新版 Tasks API，精度更高）
# ============================================================
# 用顶部已导入的 mp / mp_python / mp_vision，这里只做别名方便下面使用
_mp_python  = mp_python
_mp_vision  = mp_vision
_RunningMode = mp_vision.RunningMode

# task 文件路径：优先在脚本同目录查找，备选 uploads 路径
_TASK_CANDIDATES = [
...
if _TASK_PATH is None:
    raise FileNotFoundError(
        "找不到 hand_landmarker.task！请将文件放到脚本同目录下。"
    )
print(f"\n✓ 使用 Hand Landmarker 模型: {_TASK_PATH}")

def _make_landmarker(num_hands: int) -> _mp_vision.HandLandmarker:
    """创建 HandLandmarker（VIDEO 模式，支持帧间追踪）"""
...
# 全局 landmarker 实例（初始按双手创建，选择模式后在 main 里按需重建）
_landmarker: _mp_vision.HandLandmarker = _make_landmarker(2)

# 记录 landmarker 创建时刻，用于生成严格单调递增的真实时间戳
# VIDEO 模式要求：同一 landmarker 实例内时间戳必须严格递增（毫秒整数）
_landmarker_start_ns: int = time.perf_counter_ns()

def _now_ms() -> int:
    """返回自当前 landmarker 创建以来经过的毫秒数（严格单调递增）"""
    return int((time.perf_counter_ns() - _landmarker_start_ns) // 1_000_000)

def detect_hands(rgb_frame: np.ndarray):
    """
    对 RGB 帧做手部关键点检测。
    返回 (landmarks_list, handedness_list)
      landmarks_list : List[ List[NormalizedLandmark(x,y,z)] ]  每只手21个点
      handedness_list: List[ str ]  'Left' 或 'Right'
    使用真实 wall-clock 时间戳，让 VIDEO 模式的运动预测更准确。
    """
    mp_img = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb_frame)
    result = _landmarker.detect_for_video(mp_img, _now_ms())
    if not result.hand_landmarks:
        return [], []
    labels = [h[0].category_name for h in result.handedness]
    return result.hand_landmarks, labels

def reinit_landmarker(num_hands: int):
    """重新初始化 landmarker（切换单/双手模式时调用）。
    新实例时间戳从 0 重新计算，所以重置基准时刻即可。
    """
    global _landmarker, _landmarker_start_ns
    _landmarker.close()
    _landmarker = _make_landmarker(num_hands)
    _landmarker_start_ns = time.perf_counter_ns()   # 重置基准，新实例从 0 开始计
...
class PKHalf:
    ...
    def __init__(self, player_id: int, x_min: int, x_max: int, blade: str):
    ...
    def _spawn(self):
        ...
        if self.last_type == 'bomb' and is_bomb and random.random() < 0.5:
            is_bomb = False
        if is_bomb:
            b = Bomb('deadly' if random.random() < 0.25 else 'normal')
    ...
    def _spawn_fruit(self):
        if random.random() < 0.15 and MULTI_FRUIT_IMAGES:
            f = MultiFruit()
        else:
            f = Fruit()
        f.x = random.randint(self.x_min + 60, self.x_max - 60)
        if isinstance(f, MultiFruit):
            self.multi_fruits.append(f)
        else:
    ...
    # ---------- 碰撞 ----------
    def check_collisions(self, trail: deque):
        if len(trail) < 2:
            return
        pts = list(trail)[-5:]
    ...
    def draw(self, frame):
        ...
        for obj in self.multi_fruits:  obj.draw(frame)
        for obj in self.bombs:         obj.draw(frame)
        blade_img = BLADE_IMAGES.get(self.blade)
        for e in self.slash_fx:        e.draw(frame, blade_img)
        for e in self.juice_fx:        e.draw(frame)
...
class PKGame:
    ...
    def __init__(self, blade: str):
        half = WINDOW_WIDTH // 2
        # 玩家1：左半屏；玩家2：右半屏
        self.p1 = PKHalf(1, 0,    half,         blade)
        self.p2 = PKHalf(2, half, WINDOW_WIDTH, blade)
        self.start_time  = time.time()
        self.game_over   = False
        self.winner      = None   # 'p1' / 'p2' / 'draw'
    ...
    def check_collisions(self, trail_p1: deque, trail_p2: deque):
    ...
    def draw(self, frame):
        ...
        add_cn_text(f'{int(math.ceil(tl))}秒', (half - 55, 12),
                    font_size=52, color=timer_color,
                    bg_color=(30, 30, 30), padding=10)

        # ---- 玩家得分 HUD ----
        add_cn_text(f'玩家一', (18, 12),  font_size=30, color=(255,255,255), bg_color=(200,80,0))
        add_cn_text(f'{self.p1.score} 分', (18, 58),  font_size=42, color=(255,255,100), bg_color=(40,40,40))

        add_cn_text(f'玩家二', (half+18, 12), font_size=30, color=(255,255,255), bg_color=(0,120,220))
        add_cn_text(f'{self.p2.score} 分', (half+18, 58), font_size=42, color=(100,255,255), bg_color=(40,40,40))

        # ---- 实时领先标记 ----
        if not self.game_over:
            if self.p1.score > self.p2.score:
                add_cn_text('领先 ▶', (half - 220, 70), font_size=26, color=(255, 200, 0))
            elif self.p2.score > self.p1.score:
                add_cn_text('◀ 领先', (half + 20, 70),  font_size=26, color=(255, 200, 0))
        ...
        if self.game_over:
            ...
            cy = WINDOW_HEIGHT // 2
            if self.winner == 'p1':
                add_cn_text('🏆  玩家一获胜！', (half - 300, cy - 130), font_size=68, color=(255, 200, 0))
            elif self.winner == 'p2':
                add_cn_text('🏆  玩家二获胜！', (half - 300, cy - 130), font_size=68, color=(100, 200, 255))
            else:
                add_cn_text('势均力敌，平局！', (half - 290, cy - 130), font_size=68, color=(200, 200, 200))
...
def mode_selection_screen(cap) -> str:
    ...
    key_map = {'q': 'single', '1': 'single', '2': 'dual', '3': 'pk'}
    ...
    while True:
        ...
        # 快捷键提示
        add_cn_text('快捷键: 1=单手  2=双手  3=双人PK',
                    (WINDOW_WIDTH//2 - 230, WINDOW_HEIGHT - 55),
                    font_size=24, color=(140, 140, 140))

        rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        lm_list, _ = detect_hands(rgb)
        current_hover = None
        if lm_list:
            tip = lm_list[0][8]
        ...
        for ch, mode in key_map.items():
            if kp == ord(ch):
                cv2.destroyWindow('Swift-Fruit-Slice'); return mode
...
def main():
    ...
    n_hands = 1 if (not dual and not pk_mode) else 2

    # 按实际手数重建 landmarker
    reinit_landmarker(n_hands)

    print(f"\n✓ 游戏模式: {'双手模式' if dual else ('双人PK' if pk_mode else '单手模式')}")
    print(f"✓ 刀光: {selected_blade}")
    print(f"✓ Landmarker 已按 {n_hands} 只手重建")
    ...
    if pk_mode:
        ...
        pk_smoothers  = [FingerSmoother(method='ewma', alpha=0.4, adaptive=True) for _ in range(2)]
        pk_trails     = [deque(maxlen=15) for _ in range(2)]
        pk_last_pos   = [None, None]
        pk_finger_pos = [None, None]
        pk_game = PKGame(selected_blade)

        print("=" * 50)
        print("⚔️  双人PK模式！左半屏=玩家一  右半屏=玩家二  30秒倒计时")
        print("   按 R 再来一局 | Q 退出")
        print("=" * 50)
        ...
        while cap.isOpened():
            ...
            lm_list, label_list = detect_hands(rgb_frame)
            ...
            if lm_list:
                ...
                for lm, label in zip(lm_list[:2], label_list[:2]):
                    tip = lm[8]
                    raw_hands.append({'label': label,
                                      'rx': int(tip.x * WINDOW_WIDTH),
                                      'ry': int(tip.y * WINDOW_HEIGHT)})
                assigned = {}
                conflict = []
                for h in raw_hands:
                    slot = HAND_SLOT.get(h['label'], 0)
                    if slot not in assigned: assigned[slot] = h
                    else: conflict.append(h)
                for h in conflict:
                    free = [s for s in range(2) if s not in assigned]
                    if not free: break
                    best_s, best_d = free[0], float('inf')
                    for s in free:
                        if pk_last_pos[s]:
                            d = math.hypot(h['rx']-pk_last_pos[s][0], h['ry']-pk_last_pos[s][1])
                            if d < best_d: best_s, best_d = s, d
                    assigned[best_s] = h
                if (len(assigned) == 2 and pk_last_pos[0] and pk_last_pos[1]):
                    h0, h1 = assigned.get(0), assigned.get(1)
                    if h0 and h1:
                        d_n = (math.hypot(h0['rx']-pk_last_pos[0][0], h0['ry']-pk_last_pos[0][1]) +
                               math.hypot(h1['rx']-pk_last_pos[1][0], h1['ry']-pk_last_pos[1][1]))
                        d_s = (math.hypot(h0['rx']-pk_last_pos[1][0], h0['ry']-pk_last_pos[1][1]) +
                               math.hypot(h1['rx']-pk_last_pos[0][0], h1['ry']-pk_last_pos[0][1]))
                        if d_s < d_n - 80:
                            assigned[0], assigned[1] = h1, h0
                for slot, h in assigned.items():
                    sx, sy = pk_smoothers[slot].smooth(h['rx'], h['ry'])
                    pk_finger_pos[slot] = (sx, sy)
                    pk_trails[slot].append((sx, sy))
                    pk_last_pos[slot] = (h['rx'], h['ry'])
                    active_slots.add(slot)

            for i in range(2):
                if i not in active_slots:
                    pk_smoothers[i].reset(); pk_last_pos[i] = None

            for i in range(2):
                if pk_finger_pos[i] is None and len(pk_trails[i]) > 10:
                    for _ in range(min(5, len(pk_trails[i]))):
                        if pk_trails[i]: pk_trails[i].popleft()

            trail_colors_pk = [(120, 80, 255), (255, 80, 80)]
            for i, trail in enumerate(pk_trails):
                pts = list(trail)
                for j in range(1, len(pts)):
                    if pts[j] and pts[j-1]:
                        a = (j / len(pts)) * 0.35
                        cv2.line(frame, pts[j-1], pts[j], trail_colors_pk[i], max(2, int(2+a*6)))

            for i, pos in enumerate(pk_finger_pos):
                if pos:
                    cv2.circle(frame, pos, 25, HAND_COLORS[i], 3)
                    cv2.circle(frame, pos, 15, (0, 255, 255), cv2.FILLED)

            if not pk_game.game_over:
                pk_game.update()
                pk_game.check_collisions(pk_trails[0], pk_trails[1])
            ...
            if kp == ord('q'):
                ...
                pk_game = PKGame(selected_blade)
                for t in pk_trails:    t.clear()
                for s in pk_smoothers: s.reset()
                pk_last_pos = [None, None]
    ...
    while cap.isOpened():
        ...
        if lm_list:
            ...
            # 2. 若同一 slot 重复出现，用"与上帧位置最近"兜底
            # ============================================================
            raw_hands = []
            for lm, label in zip(lm_list[:n_hands], label_list[:n_hands]):
                tip = lm[8]   # 食指尖（landmark index 8）
                raw_hands.append({
                    'label': label,
                    'rx': int(tip.x * WINDOW_WIDTH),
                    'ry': int(tip.y * WINDOW_HEIGHT),
            ...
            if n_hands == 1:
                h = raw_hands[0]
                sx, sy = smoothers[0].smooth(h['rx'], h['ry'])
                finger_pos[0]   = (sx, sy)
                finger_speed[0] = math.hypot(h['rx'] - (last_pos[0][0] if last_pos[0] else h['rx']),
                                             h['ry'] - (last_pos[0][1] if last_pos[0] else h['ry']))
                trail_list[0].append((sx, sy))
                last_pos[0] = (h['rx'], h['ry'])
                active_slots.add(0)
                ...
                for h in raw_hands:
                    slot = HAND_SLOT.get(h['label'], 0)
                    if slot not in label_assigned:
                        label_assigned[slot] = h
                    else:
                        conflict.append(h)

                # 步骤2：冲突手分配到空闲 slot（按上帧距离）
                for h in conflict:
                    free = [s for s in range(n_hands) if s not in label_assigned]
                    if not free:
                        break
                    best_slot = free[0]
                    best_dist = float('inf')
                    for s in free:
                        if last_pos[s]:
                            ...
                            if d < best_dist:
                                best_dist = d; best_slot = s
                    label_assigned[best_slot] = h

                # 步骤3：位置互换校验（防止 handedness 误判导致轨迹跳变）
