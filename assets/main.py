...
class FingerSmoother:
    ...
    def _kalman_update(self, kalman, measurement):
        ...
        # 预测协方差更新 (简化版)
        kalman['P'][0][0] += kalman['P'][1][1] + kalman['Q']
        kalman['P'][1][1] += kalman['Q']
        
        # 2. 更新 (测量)
        # 计算卡尔曼增益 (简化版)
        S = kalman['P'][0][0] + kalman['R']
        K_pos = kalman['P'][0][0] / S
        K_vel = kalman['P'][1][1] / S # 简单的速度增益
        
        # 更新状态
        error = measurement - kalman['x']
        kalman['x'] = kalman['x'] + K_pos * error
        kalman['v'] = kalman['v'] + K_vel * error
        
        # 更新协方差 (简化版)
        kalman['P'][0][0] = (1 - K_pos) * kalman['P'][0][0]
        kalman['P'][1][1] = (1 - K_vel) * kalman['P'][1][1]
